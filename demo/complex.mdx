...
<Init
  state={{
    vehicles: [
      ...
      { name: "Volvo", price: 31000, reliability: 7, luxury: 8 },
      { name: "Mercedes", price: 45000, reliability: 6, luxury: 10 }
    ]
  }}
  key="complex-example"
/>
...
<Observe>
  {
    ({ state, setState }) => {
      const specialFormula = ({ price, reliability, luxury }) =>
        (reliability * 3 + luxury * 2) / price;
      const byFormula = [...state.vehicles]
        .map(v => ({ ...v, score: specialFormula(v) }))
      ...
      return ( 
        <VictoryChart domainPadding={20} theme={VictoryTheme.material}>
          ...
          />
          <VictoryAxis />
          <VictoryBar data={byFormula} x="name" y="score" fill="blue" />
        </VictoryChart>
      )
    }
  }
